...
***Βήμα 3ο***  
Είναι γεγονός πως η αύξηση του μεγέθους των caches, επιφέρει σημαντικό κόστος στον χρόνο σάρωσης των caches. Παρόλα αυτά παρατηρούμε πως με την αύξηση αυτή έχουμε καλύτερη απόδοση. Σκοπός λοιπόν είναι να βρούμε το σημείο όπου από τη μία, ο χρόνος προσπέλασης δεν αυξάνεται σημαντικά και από την άλλη, οι αστοχίες μειώνονται. Συγκεκριμένα, καθώς ο χρόνος προσπέλασης της L1 είναι πιο κρίσιμος από αυτόν της L2, το κόστος της αύξησης του μεγέθους της είναι μεγαλύτερο. Ακόμη, η L1i είναι πιο μικρή L1d, γιατί φορτώνονται σε αυτήν οι εντολές που είναι πιο μικρές σε μέγεθος από τα δεδομένα.  
...
Τέλος, οι βέλτιστες αρχιτεκτονικές για το κάθε benchmark:  
* bzip: l1d=128kB l1i=32kB l1d_assoc=256 l1i_assoc=256 l2_assoc=256 cacheline=512 l2=4MB   
* sjeng: l1d=64kB l1i=128kB l1d_assoc=16 l1i_assoc=16 l2_assoc=512 cacheline=2048 l2=4MB  
* hmmer: l1d=128kB l1i=128kB l1d_assoc=64 l1i_assoc=64 l2_assoc=64 cacheline=512 l2=4MB     
* libm: l1d=64kB l1i=16kB l2=512kB l1d_assoc=64 l1i_assoc=64 l2_assoc=32 cacheline=2048  


***Κριτική***   
Η εργασία αυτή ήταν πιο ενδιαφέρουσα από την προηγούμενη, καθώς αντιμετωπίσαμε πιο πρακτικά θέματα αρχιτεκτονικής. Βέβαια, ήταν πιο απαιτητική στο χρόνο που χρειάστηκε για την ολοκλήρωσή της. Επίσης, δε μπορέσαμε να ολοκληρώσουμε όλες τις απαραίτητες αλλαγές στο mcf και δεν μας ήταν ξεκάθαρο εξ'αρχής τι έπρεπε να κάνουμε στην συνάρτηση κόστους.   
